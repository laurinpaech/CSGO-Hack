...
public:
    ...
    type read(mach_vm_address_t address, size_t extraSize = sizeof(type), bool debug = false)
    {
        ...
        kern_return_t re = vm_read(mainTask, (vm_address_t)address, extraSize, &data, &sz);
        if(re == KERN_SUCCESS)
        {
    ...
    template <typename type>
    type read2(mach_vm_address_t address, size_t extraSize = sizeof(type), bool debug = true)
    {
        type content;
        ...
        if(debug) {
            ...
            std::cout << std::hex << (type *) data << std::endl;
    ...
    uint8_t * readData(vm_address_t moduleStart, size_t moduleSize)
    {
        ...
        kern_return_t re = vm_read(mainTask, (vm_address_t) moduleStart, sizeMax, &readMem, &sizeMax);
    ...
    bool write(mach_vm_address_t address, type data, bool debug = false)
    {
        ...
        kern_return_t re = vm_write(mainTask, address, (vm_offset_t)&data, sizeof(data));
        if (re == KERN_SUCCESS)
        {
            return true;
        }
